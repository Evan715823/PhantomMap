...
\caption{\textbf{Top:} phantom boxes that Qwen2.5-VL and LLaVA-NeXT
drew around objects that are \emph{not} in the image.
\textbf{Bottom:} boxes the same two models drew for objects that
\emph{are} in the image. The eye already distinguishes phantoms
(smaller, off-centre) from honest boxes (larger, naturally
distributed).}
\label{fig:teaser}
\end{figure}
...
baseline we know of. DASH \cite{dash} clusters \emph{images} that
systematically trigger hallucinations. Visual CounterFact
\cite{pixelsvspriors} studies the pixel-vs-prior conflict. Unlike all
these, PhantomMap operates on the VLM's \emph{prompted bbox output} --
the channel actually exposed to a user -- and is the first to profile
the \emph{spatial distribution} of hallucinated objects.
...
\caption{PhantomMap pipeline. For each $(I, o)$ pair in POPE, the VLM
is prompted for yes/no plus a bbox. When POPE labels the object
absent, every ``yes'' is a phantom event and its bbox feeds both the
atlas (\S\ref{sec:atlas}) and the detector (\S\ref{sec:detector}).}
\label{fig:method}
\end{figure}
...
fraction of ``yes'' answers that yielded a parseable bbox. Qwen is
more willing to ground and more willing to hallucinate; LLaVA is the
opposite.}
\label{tab:rates}
\end{table*}
...
\caption{\textbf{Phantom atlas.} KDE of phantom-box centres vs
honest-box centres, per model. Phantom boxes are displaced
\emph{outward} relative to the honest distribution for both models.
Colour = density; white dots = individual events.}
\label{fig:atlas}
\end{figure*}
...
chance. Axes are false-positive and true-positive rates.}
\label{fig:roc}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Hallucination-detection AUROC on stratified $70/30$ splits
over ``yes'' events that yielded a valid bbox ($n{=}6{,}965$ Qwen;
$n{=}812$ LLaVA). Our first-party detector exceeds HALP's
internal-feature detector on Qwen by $+12.8$ AUROC points.}
\label{tab:detector}
\end{table}
...
(Fig.~\ref{fig:failures}): oversized hedge boxes that half-cover the
scene; peripheral boxes pressed to a corner; and edge-clipped boxes
flush with an image border.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figures/fig5_failures.pdf}
\caption{\textbf{Failure-mode taxonomy.} Top: phantoms covering most
of the image. Middle: phantoms pressed against the periphery. Bottom:
phantoms flush with an image border.}
\label{fig:failures}
\end{figure}
...
brainstorming, literature-scan sanity checks, and boilerplate-code
drafting; every experimental choice, run, and figure is our own. We
thank the authors of POPE \cite{pope}, Qwen2.5-VL \cite{qwen25vl},
LLaVA-NeXT \cite{llavanext}, and EAZY \cite{eazy}. Author~1 wrote the
evaluation code and ran all VLM experiments; Author~2 built the atlas
visualisations, produced the figures, and wrote the related work.
