...
- Brainstormed candidate project topics with the constraint *vision +
  ...
  whose idea overlapped directly with already-published work.
...
- We used Claude's web-search tools to find adjacent papers (EAZY,
...
- Inventing experimental results. Every number in the report comes
  from a real run; raw outputs are committed to `results/`.
